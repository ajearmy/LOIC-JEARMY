...
namespace LOIC
{
	...
	{
		const string AttackText = "هل تريد البدء";
		const string StpFldText = "إيقاف التدفق";
			...
			if((cmdAttack.Text == AttackText && toggle) || (!toggle && on))
			{
				try
				{
					...
					if (String.IsNullOrEmpty(sTargetIP) || String.IsNullOrEmpty(sTargetHost) || String.Equals(sTargetIP, "N O N E !"))
						throw new Exception("إختر الهدف");
		...
		private void LockOnURL(bool silent = false)
		{
			try
			{
				...
				if(tURL.Length == 0)
				{
					Wtf ("خطأ في الرابط، حاول مرة أخرى..", silent);
					return;
				}
				if(!tURL.Contains("://"))
				{
		...
		{
			this.Text = String.Format("{0}  | النسخة المحدثة", Application.ProductName, Application.ProductVersion);
		}
		...
		{
            Process.Start("https://jearmy.org");
		}
